...
В данной версии моего котейнера были приняты следующие допущения:

1. Пока что контейнер не поддерживает создание singleton бинов.
2. Не провидится проверка на цикличесике зависимости между бинами (Одному объекту надо передать в конструтор второй объект, а второму требуется передать в конструтор первый). 
3. Считается, что все типы параметров бинов являются интерфесами, если нет, то выбрасывается исключение.
4. Не проводится проверка параметров конструктора на соответсвии условиям использования контейнера.